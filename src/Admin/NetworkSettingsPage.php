...
namespace AzureInsightsMonolog\Admin {
	...
	class NetworkSettingsPage {
		...
		private function tab_behavior(): void {
			...
			$recBatchSize  = defined( 'AIW_DEFAULT_BATCH_MAX_SIZE' ) ? (int) AIW_DEFAULT_BATCH_MAX_SIZE : 20;
			$recFlush      = defined( 'AIW_DEFAULT_BATCH_FLUSH_INTERVAL' ) ? (int) AIW_DEFAULT_BATCH_FLUSH_INTERVAL : 5;
			$this->field_number( 'aiw_batch_max_size', 'Batch Size', (int) $this->so( 'aiw_batch_max_size', $recBatchSize ), 1, 100, 'Maximum telemetry items per batch. Recommended default: ' . $recBatchSize );
			$this->field_number( 'aiw_batch_flush_interval', 'Flush Interval (s)', (int) $this->so( 'aiw_batch_flush_interval', $recFlush ), 1, 300, 'Auto-flush incomplete batches after this many seconds. Recommended default: ' . $recFlush );
			...
			$recHook = defined( 'AIW_DEFAULT_SLOW_HOOK_THRESHOLD_MS' ) ? (int) AIW_DEFAULT_SLOW_HOOK_THRESHOLD_MS : 150;
			$recQuery = defined( 'AIW_DEFAULT_SLOW_QUERY_THRESHOLD_MS' ) ? (int) AIW_DEFAULT_SLOW_QUERY_THRESHOLD_MS : 500;
			$this->field_number( 'aiw_slow_hook_threshold_ms', 'Slow Hook Threshold (ms)', (int) $this->so( 'aiw_slow_hook_threshold_ms', $recHook ), 10, 5000, 'Record hook metrics above this duration. Recommended default: ' . $recHook );
			$this->field_number( 'aiw_slow_query_threshold_ms', 'Slow Query Threshold (ms)', (int) $this->so( 'aiw_slow_query_threshold_ms', $recQuery ), 10, 10000, 'Record db_slow_query_ms metrics above this duration. Recommended default: ' . $recQuery );
			echo '</table></div>';
